...
declare -g global_theme="dark" # fallback

if [[ -f "$STATE_FILE" ]]; then
    global_theme="$(<"$STATE_FILE")"
fi

# 1. Map flags to mode names
case "$1" in
-l | light) mode="light" ;;
-d | dark) mode="dark" ;;
-t | toggle) [[ "$global_theme" == "dark" ]] && mode="light" || mode="dark" ;;
*)
    echo "Usage: $0 {-d|light|-l|dark|-t|toggle} [-f: Force]"
    exit 1
    ;;
esac

apps=(_xmonad _emacs _rofi _xmobar _xresources _starship _zathura _dunst _system_theme _xsettingsd)
